...
from apps.detector.forms import DeleteForm, DetectorForm, UploadImageForm # DeletagForm
...
def index():
    ...
    # DeletagFormをインスタンス化する
    # deletag_form = DeletagForm()
...
"""
@dt.route("/images/deletag/<string:image_id>", methods=["POST"])
# login_requiredデコレーターをつけてログイン必須とする
@login_required
def deletag(image_id):
    # user_imagesテーブルからレコードを取得する
    user_image = db.session.query(UserImage).filter(UserImage.id == image_id).first()
    if user_image is None:
        flash("タグ削除対象の画像が存在しません。")
        return redirect(url_for("detector.index"))
